...
/**
 * Activity: Fetch hotels from Supplier A via HTTP
 */
export async function fetchSupplierAActivity(city?: string): Promise<SupplierHotelRaw[]> {
  ...
  try {
    const res = await axios.get<SupplierHotelRaw[]>(url, { timeout: 5000 });
    console.log(`[Temporal Activity] Supplier A returned ${res.data.length} hotels.`);
    return res.data;
  } catch (error) {
    console.error(`[Temporal Activity] Supplier A fetch failed: ${(error as Error).message}`);
    throw error;
  }
}

/**
 * Activity: Fetch hotels from Supplier B via HTTP
 */
export async function fetchSupplierBActivity(city?: string): Promise<SupplierHotelRaw[]> {
  ...
  try {
    ...
    console.error(`[Temporal Activity] Supplier B fetch failed: ${(error as Error).message}`);
    throw error;
  }
}
